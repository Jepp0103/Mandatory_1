//jQuery hide and show buttons for the commandsAndTools page
   //Hiding all paragraphs by default when entering page
  //Node paragraph
    ...
    $(nodeButton).click(() => {
     ...
     //npm paragraph
     var npmButton = "#npmButton"
     var npmInfo = "#npmInfo, #npmRunInfo, #npmInitInfo"
     $(npmButton).css("background-color", "#ff3333");
     var npmClickCount = 0;
     $(npmButton).text("Hide npm paragraph");
 
     $(npmButton).click(() => {
         console.log(npmClickCount);
 
         if (npmClickCount % 2 == 0) {
           $(npmInfo).toggle();
           $(npmButton).text("Show npm paragraph");
           $(npmButton).css("background-color", "Chartreuse");
         }
